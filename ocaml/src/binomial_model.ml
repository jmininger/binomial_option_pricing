...
let create_recombinant spot_price ~up ~prob_up =
  if up <= 1. then raise (Failure "up factor can not be < 1")
  else if prob_up > 1. || prob_up < 0. then
    raise (Failure "prob_up must be a valid probability")
  else {spot_price; up; prob_up; down= 1. /. up; prob_down= 1. -. prob_up}
...
let martingale_probability rate ~up ~down ~delta_t =
  ...
  if prob_u >= 0. && prob_u <= 1. then (prob_u, 1. -. prob_u)
  else
...
let asset_states {spot_price; up; down; _} ~periods =
  let indices =
    Sequence.range 0 periods ~stop:`inclusive |> Sequence.map ~f:float
  in
  let periods = float periods in
  let prices =
    Sequence.map indices ~f:(fun idx ->
        spot_price *. (up ** idx) *. (down ** (periods -. idx)) )
  in
...
let price_level future_prices ~prob_up ~rate ~delta_t =
  ...
  let discounted_expec low high =
    let expect =
      binomial_expectation high low ~p_high:prob_up ~p_low:prob_down
    in
...
let price_option opt_type strike ~spot ~res ~rate ~time ~variance =
  ...
  let prob_up, _ = martingale_probability rate ~up ~down ~delta_t in
